...
object Vehicle{
  def apply(vehicleType: String, vehicleId:String): Option[Vehicle] ={
      if("car".equalsIgnoreCase(vehicleType)) return id(vehicleId).map( id => Car(id) )
      if("motorbike".equalsIgnoreCase(vehicleType)) return id(vehicleId).map( id => Motorbike(id))
     None
  }
  def id(id:String):Option[VehicleId] = if(!id.trim.isEmpty) Some(VehicleId(id)) else None
}
